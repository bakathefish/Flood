...
4. [x] **Verification.** `snowmelt` joins the variant table for Bhakra only; `verify.variant_verdict(..., dams=("Bhakra",))`; `results.json["snowmelt_verdict"]`; a paragraph and the row in `verification.md`; `design.md` states the term and the rule; the roadmap records the outcome.
