...
5. [x] **Verification and report.** Outcome 2026-09-17: both carriers REFUSED under the rule (held-out error unchanged, season peaks lower, Pong's gamma negative); the product keeps `api`. The three variants in `inflow_variants.csv`, the verdict in `results.json`, a paragraph and the table in `verification.md`; `design.md` documents the carrier and the rule; the roadmap moves item 6 to Done with the outcome.
