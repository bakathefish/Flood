...
Run from ``punjabflood/``:
  ...
  python scripts/pull_snow_points.py --points 112,113,114
"""
...
def main() -> None:
    ...
    if a.list_missing:
        missing = [
            ...
            if not client._cache_path("archive", _params(lat, lon)).exists()
        ]
    ...
    for i in idx:
        ...
        j = client.archive_daily(lat, lon, START, END, daily=snow.ARCHIVE_DAILY)
