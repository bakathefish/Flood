...
- **Forecasting, three days out.** For every district, every day of the monsoon, using only what is known when the forecast is issued: will the satellite see flooding here within the next three days? Gradient boosting over a decade of daily Copernicus flood observations, with history features that carry recent flooding across neighbouring districts, decaying over about three days. Under walk-forward simulation, where each season is forecast using only seasons that already happened, it had the best pooled ranking of the candidates we tested: average precision 0.249 against a 3.07% base rate, 8.1 times lift. Resampling takes most of that away and we publish the result rather than the headline: remove 2025 alone and it falls to 0.042, a season-block bootstrap puts a 95% interval of [0.004, 0.521] around it, and by median season the same model without its excitation features is better (0.133 against 0.083). The excitation gain is +0.093 with interval [-0.002, +0.268], ahead in 90% of resamples: probable, not established. Season-level performance is heterogeneous and it does not lead every comparator in every flood season: it wins decisively in 2025 (0.536 against 0.100 for persistence), and trails other candidates in 2019, 2022 and 2023, where the same model without the history features scores 0.243 against its 0.145. The ranking is the solid result; the alerting is modest and reported as such: about 24 alerts a season, roughly one in three followed by flooding, catching about one onset in four, with the alert level set inside each fold rather than from scores the model had already fitted. This is a selective trigger for recorded satellite flood detections, not a comprehensive warning system. All of it is retrospective and post-selection; episode-clean and prospective validation are outstanding, and the 2026 monsoon is the prospective test. Method, benchmark and the numbers we had to retract: `docs/notes/forecaster.md`.
