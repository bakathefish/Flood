...
- **Crop impact:** 36,195 ha of cropland flooded in the peak-window snapshot ≈ **₹351–523 crore** paddy damage band (duration-weighted → at-risk; DES district yields × MSP; official cumulative-season figure is 1.48–1.75 lakh ha — divergence explained in the method notes).
...
- **Forecaster:** XGBoost on the pipeline's own 2,420 SAR-derived labels; leave-one-year-out ROC-AUC 0.946 (leakage-checked). Trained on 2015–2024 only, it flagged all five of the SAR atlas's most-flooded districts (girdawari-corroborated; Kapurthala #1) with ~10 days of pre-crest lead on the dam-release peak. SHAP: antecedent flooding, flood-history prior, Bhakra storage — ablation-tested: the 2025 flags are unchanged with every reservoir feature deleted (attribution, not load-bearing skill), and the persistence comparison is published verbatim (`docs/notes/ablation.md`).
...
- **The gap, quantified:** Punjab has **zero CWC flood-forecast stations** — absent from CWC's own state-wise table (226 stations, 22 states) and from its current lists (`docs/notes/cwc-gap.md`); the 70-year public damage record is sparse milestones, charted honestly (`atlas/punjab_flood_history.png`).
