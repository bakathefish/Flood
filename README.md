...
- **Forecasting, three days out.** For every district, every day of the monsoon, using only what is known when the forecast is issued: will the satellite see flooding here within the next three days? Gradient boosting over a decade of daily Copernicus flood observations, with history features that carry recent flooding across neighbouring districts, decaying over about three days. Under walk-forward simulation, where each season is forecast using only seasons that already happened, it ranked flood risk better than persistence, a transparent climatology-plus-neighbour rule, and penalised logistic regression, in every season that contained flooding. In 2025, the season with the most flood days in the record, average precision was 0.549 against 0.096 for persistence. The ranking is the solid result; the alerting is modest and reported as such: about 28 alerts a season, roughly one in three followed by flooding, catching about one onset in four. All of it is retrospective and post-selection, and the 2026 monsoon is the prospective test. Method, benchmark and the numbers we had to retract: `docs/notes/forecaster.md`.
