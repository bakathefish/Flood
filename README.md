...
- **Risk ranking, held out, and its limits.** Trained on 2015 to 2024 only, the model ranked 2025's worst-flooded districts at the top, Kapurthala first. Honest recall is five of the six districts that flooded (Jalandhar was missed). It does not beat a one-line persistence baseline ("it flooded last window"), and a re-audit found two reasons why. First, the published evaluation leaked: district priors were computed over the whole record and joined before the year-held-out split. Second, 17 of the 27 flood windows were continuation of water already on the ground, not onset, so most of the apparent skill was noticing water that was already there. Rebuilt fold-safe and scored on onset only, no learned variant beats persistence once the model is chosen strictly inside each fold. Persistence is the operational baseline; the learned model is experimental. No lead-time claim is made. The whole audit ships in `docs/notes/forecaster-v2.md`.
